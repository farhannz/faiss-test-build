...
RUN git clone -b v1.8.0 https://github.com/facebookresearch/faiss.git && \
    cd faiss &&\
    MKLROOT=/opt/intel/oneapi/mkl/2024.1 cmake -B build -DBUILD_TESTING=ON -DBUILD_SHARED_LIBS=ON -DFAISS_ENABLE_GPU=ON -DFAISS_ENABLE_RAFT=OFF -DPYTHON_EXECUTABLE=$(which python3) -DCMAKE_BUILD_TYPE=Release -DBLA_VENDOR=Intel10_64_dyn DCUDAToolkit_ROOT=/usr/local/cuda -DCMAKE_CUDA_ARCHITECTURES="75;70;61;80;86;89;90" . &&\
